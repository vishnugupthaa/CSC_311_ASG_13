...
public class WikiSearch {
	...
	public WikiSearch or(WikiSearch that) {
		// TODO: FILL THIS IN!
        Map<String, Integer> result = new HashMap<>(this.map);
        for (String url : that.map.keySet()) {
            int relevance = totalRelevance(this.getRelevance(url), that.getRelevance(url));
            result.put(url, relevance);
        }
	...
	public WikiSearch minus(WikiSearch that) {
        ...
        for (String url : this.map.keySet()) {
            if (!that.map.containsKey(url)) {
                result.put(url, this.getRelevance(url));
            }
        }
        return new WikiSearch(result);
		// return null;
